        ...
        if (*fmt == '%')  //%开头的一般都是需要被可变参数 取代的
        {
            ...
            switch (*fmt)
            {
                ...
                case 'f': //一般 用于显示double类型数据，如果要显示小数部分，则要形如 %.5f
                    ...
                    //正整数部分，先显示出来
                    buf = sprintf_num(buf, last, ui64, zero, 0, width); //把一个数字 比如“1234567”弄到buffer中显示
                    ...
                    if (frac_width) //指定了显示多少位小数
                    {
                        ...
                        buf = sprintf_num(buf, last, frac, '0', 0, frac_width); //frac这里是小数部分，显示出来，不够的，前边填充'0'字符
                    }
                    fmt++;
                    continue;  //重新从while开始执行
...
static u_char * sprintf_num(u_char *buf, const u_char *last, uint64_t ui64, u_char zero, uintptr_t hexadecimal, uintptr_t width)
{
    //temp[21]
